  ...
  return (
    <span
      ...
      className={`text-xs px-2.5 py-1.5 border rounded-lg flex items-center gap-2 transition-all ${
        ...
        status === 'available' ? 'bg-emerald-50/60 dark:bg-emerald-950/20 border-emerald-100 dark:border-emerald-900 text-emerald-700 dark:text-emerald-400' :
        'bg-rose-50/60 dark:bg-rose-950/20 border-rose-100 dark:border-rose-900 text-rose-700 dark:text-rose-400 line-through'
      }`}
      title={status === 'idle' ? "Click to check availability" : undefined}
    >
      <span>{domain}</span>
